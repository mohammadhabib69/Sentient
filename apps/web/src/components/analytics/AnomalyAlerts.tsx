...
export function AnomalyAlerts() {
  const anomalies = [
    ...
    { id: 2, title: 'Database latency elevated', agent: 'System', severity: 'medium', time: '1h ago' },
    { id: 3, title: 'Multiple webhook failures', agent: 'Aria', severity: 'medium', time: '3h ago' },
    { id: 4, title: 'Config override detected', agent: 'Mohammad', severity: 'low', time: '5h ago' },
  ]
    ...
    <div className="flex h-full flex-col rounded-xl border border-[var(--glass-border)] bg-[var(--surface-1)] shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {anomalies.map(anomaly => (
          <div key={anomaly.id} className="flex items-start gap-3 rounded-lg border border-[var(--glass-border)] bg-[var(--surface-2)] p-3">
            <AlertTriangle className={`mt-0.5 size-4 shrink-0 ${anomaly.severity === 'high' ? 'text-[var(--red)]' : 'text-[var(--amber)]'}`} />
            ...
            <span className={`shrink-0 rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide border ${severityColors[anomaly.severity]}`}>
              {anomaly.severity}
            </span>
          </div>
        ))}
