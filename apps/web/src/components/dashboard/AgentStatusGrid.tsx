      ...
      <div className="grid grid-cols-2 gap-4">
        {agents.map((agent) => {
          ...
          return (
            <div
              key={agent.id}
              className="flex flex-col rounded-lg border border-[var(--glass-border)] bg-[var(--surface-2)] p-4"
            >
              <div className="flex items-center justify-between">
                <div
                  className="flex size-8 items-center justify-center rounded-md bg-opacity-20"
                  style={{
                    backgroundColor: `color-mix(in srgb, hsl(${color}) 20%, transparent)`,
                    color: `hsl(${color})`,
                  }}
                >
              ...
              <div className="mt-3">
                ...
                <p className="text-xs text-[var(--foreground-3)]">
                  {agent.actionsCount.toLocaleString()} actions
                </p>
              </div>
            </div>
          );
