...
export function MetricCard({ title, value, subtext, icon: Icon, amberTint, onClick }: MetricCardProps) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className={cn(
          ...
          amberTint ? "bg-[var(--amber)] text-[var(--amber)]" : "bg-[var(--foreground-3)] text-[var(--foreground-2)]"
        )}>
      ...
      <div className="mt-4">
        ...
        {subtext && (
          <p className="mt-1 text-xs text-[var(--foreground-3)]">{subtext}</p>
        )}
      </div>
