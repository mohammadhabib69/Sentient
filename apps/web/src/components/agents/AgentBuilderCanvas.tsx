...
function TriggerNode({ data, selected }: { data: any; selected?: boolean }) {
  return (
    <div className={`w-[220px] rounded-xl overflow-hidden shadow-lg border transition-all duration-200 ${
      selected 
...
function ConditionNode({ data, selected }: { data: any; selected?: boolean }) {
  return (
    <div className={`w-[220px] rounded-xl overflow-hidden shadow-lg border transition-all duration-200 relative ${
      selected 
        ? "border-[hsl(var(--primary))] ring-1 ring-[hsl(var(--primary))]/30 scale-[1.02]" 
        : "border-border bg-card"
    }`}>
...
const initialNodes: Node[] = [
  ...
  },
  {
    ...
    data: { 
      ...
      model: "GPT-4o",
      systemPrompt: "You are a helpful AI coordinator. Analyze the overdue tasks and reassign them to the best suited team member based on past workload and expertise.",
  ...
  },
  {
    ...
    data: { label: "Send Email", body: "To: manager@company.com · Subject: All tasks on track ✓", icon: "✉️" },
  },
];
...
function CanvasContent() {
  ...
  const onConnect: OnConnect = useCallback(
    (connection) => {
      saveStateToHistory();
      const edgeColorClass = connection.sourceHandle === "false" ? "animated-flow-false" : "animated-flow";
      ...
      <div className="flex flex-1 relative overflow-hidden bg-[#EAEEF0] dark:bg-[#16191A]">
        ...
        <aside className="w-[240px] border-r border-[var(--glass-border)] bg-[var(--surface-2)]/95 backdrop-blur-xl p-4 flex flex-col gap-5 z-10 select-none overflow-y-auto shrink-0 shadow-lg">
          ...
          <div className="flex flex-col gap-2">
            <h4 className="text-[10px] font-bold text-[var(--foreground-2)] tracking-wide uppercase">Triggers</h4>
          ...
          {/* Condition Section */}
          <div className="flex flex-col gap-2">
            <h4 className="text-[10px] font-bold text-[var(--foreground-2)] tracking-wide uppercase">Conditions</h4>
            <div className="flex flex-col gap-1.5">
              {[
          ...
          {/* Action Section */}
          <div className="flex flex-col gap-2">
            <h4 className="text-[10px] font-bold text-[var(--foreground-2)] tracking-wide uppercase">Actions</h4>
            <div className="flex flex-col gap-1.5">
              {[
          ...
          {/* Data Section */}
          <div className="flex flex-col gap-2">
            <h4 className="text-[10px] font-bold text-[var(--foreground-2)] tracking-wide uppercase">Data</h4>
            <div className="flex flex-col gap-1.5">
              {[
        ...
        <aside className={`w-[320px] border-l border-[var(--glass-border)] bg-[var(--surface-2)]/95 backdrop-blur-xl p-4 flex flex-col gap-4 z-10 overflow-y-auto shrink-0 shadow-lg transition-transform duration-200 ${
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2 bg-[var(--surface-1)] border border-[var(--glass-border)] rounded-lg p-2.5 shadow-sm">
                  <div className="size-7 rounded bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))] flex items-center justify-center font-bold text-sm">
                    {selectedNode.type === "triggerNode" ? "⏰" : selectedNode.type === "conditionNode" ? "⚖️" : selectedNode.type === "aiPromptNode" ? "🤖" : "⚙️"}
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-foreground">{nodeData.label}</span>
                    <span className="text-[10px] text-[var(--foreground-3)] font-medium font-mono">{selectedNode.id}</span>
                  </div>
                </div>
                ...
                {selectedNode.type === "aiPromptNode" ? (
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-wrap gap-1 border border-dashed border-[var(--glass-border)] rounded-lg p-2 bg-[var(--surface-1)]/50">
                          <span className="text-[9px] font-bold text-[var(--foreground-3)] w-full mb-0.5">Drag-in Variables:</span>
                          {[
                            "{{overdue_tasks}}",
                            "{{team_lead}}",
                            "{{current_time}}"
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold font-mono text-[hsl(var(--primary))]">
                          {(nodeData.temperature !== undefined ? nodeData.temperature : 0.7).toFixed(1)}
                        </span>
                      </div>
                      <input
                        type="range"
                    ...
                    <div className="border-t border-[var(--glass-border)] pt-4 mt-2">
                      <Button variant="outline" className="w-full text-xs font-semibold gap-1.5 shadow-sm py-2">
